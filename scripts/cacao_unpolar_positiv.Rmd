...
This script can be used for the analysis and the random forest of one specific group (polar_positiv, polar_negativ, ...)
...
```{r param}
# folder with data
filepath <- "/Volumes/Transfer/20180503_cacao/analyse_2/unpolar_positiv_short_script/"

# from ExtractConsensusXML (KNIME)
inputfile <- "BKQS_consensusXML.csv"

# from getNamesConsensusXML (python)
barcodefile <- "BKQCS_names.csv"

# file with barcodes and metadata used for metadata-registration
registrationfile <- "Herkunftsproben_und_Rostreihe.txt"
...
filepath_mapping = "BKQCS_mapping_probenart.csv"
filepath_mapping_2 = "BKQCS_mapping_herkunft_probenart.csv"

# filters applied after normalization
filterRoest = F #this time without Filter 
...
# vector of rownames which are over 99% NA and/or have higher intenisty as 0.8*10^5
...
# non-numeric values allowed in PCA 
pca$Probenart<- NULL 
...
plot_probenart <- autoplot(pca_t, 
                           data = tdS_norm_ln, 
                           colour = 'Probenart') +
                           ggtitle("Hauptkomponentenanalyse Probenart (unpolar positiv)") +
                           theme(plot.title = element_text(size = 10, hjust = 0.5),
                                 axis.title.x = element_text(size = 8),
                                 axis.title.y = element_text(size = 8))

if(T){
  ggsave("~/Desktop/pca_probenart_unpolar_positiv_new.png", plot = plot_probenart, width = 15, height = 7.5, units = "cm")
}
...
if(T)
{
  png("~/Desktop/Tree_unpolar_positiv_new.png", width = 1200, height = 1200)
  plot(tree,gp = gpar(fontsize = 20), main = "Entscheidungsbaum (unpolar positiv)")
  dev.off()
}
...
testPattern <- "_001\\.|_003\\.|_010\\.|_029\\.|_012\\.|_017\\.|_024\\.|_027\\.|_030\\.|_034\\.|_046\\.|_048\\.|_R2\\.|_R5\\."
...
if(export_boruta)
{
  ...
  write.csv(dsnB,file = paste(filepath,"/summary/","analysis_boruta_maxruns1000_all_unpolar_pos",".csv",sep=""), fileEncoding = "UTF-16LE")
...
v.table <- venn(list("b"=rownames(boruta.confirmed), "rfe" = predictors(rfe.train), "rf" = predictors(rf.train)))
print(v.table)

if(T)
{
  png("~/Desktop/venn_unpolar_positiv_new.png", width = 20, height = 20, units = "cm", res = 300)
  plot(v.table)
  title(main = "Überlapp verschiedener Klassifikationen (unpolar positiv)")
  dev.off()
}
```
